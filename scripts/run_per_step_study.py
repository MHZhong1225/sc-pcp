...
python scripts/run_per_step_study.py \
  ...
  --seeds 0:20 --devices cuda:0,cuda:1
...
def run_setting(config: ExperimentConfig, output_dir: Path) -> None:
    jobs = [(seed, config.devices[index % len(config.devices)]) for index, seed in enumerate(config.seeds)]
    try:
        # Keep one persistent process on one physical GPU for the full setting.
        # This avoids cross-device CUDA contexts when jobs have unequal runtimes.
