...
这就是 Table 1/2、Figure 2 和正文 target eligibility 使用的 WSC。
...
随机流由 dataset 和 method 确定，因此重复渲染可复现。Figure 2 对同一方法的
不同 feedback levels 复用同一 seed-resampling stream，以保持 paired 结构。
...
Figure 1 的逐时点 coverage/width error bars 也使用跨 seed 的 pointwise
