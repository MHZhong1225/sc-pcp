...
def test_start_objective_tie_resolves_in_input_order() -> None:
    first = _start("profiled", (1.0,), width=(1.0,))
    second = _start("greedy", (2.0,), width=(1.0,))
...
def test_reduced_landscape_matches_exhaustive_best_and_improves_all_starts() -> None:
    ...
    starts = (
        ...
        _start("upper", (0.0, 2.0, 0.0), width=(4.0, 4.0, 4.0)),
    )
    ...
    exhaustive_width = min(float(widths[schedule].item()) for schedule in all_27_schedules)
...
def test_resume_continues_at_pair_five_and_returns_only_pair_eight() -> None:
    ...
    outcome = resume_cyclic_joint_coordinate_search(
        (_pair_four_state("profiled"), _pair_four_state("greedy")),
        torch.tensor([[1.0, 2.0]]),
        no_feasible_proposals,
    )
...
@pytest.mark.parametrize(
    "states",
    [
        (_pair_four_state("profiled", 3), _pair_four_state("greedy", 3)),
        (_pair_four_state("profiled", 4), _pair_four_state("greedy", 3)),
    ],
)
